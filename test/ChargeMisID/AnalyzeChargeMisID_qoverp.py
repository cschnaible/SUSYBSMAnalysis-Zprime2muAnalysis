...
parser.add_argument('-r','--refit',default='tuneP',help='Which muon refit to compare to gen')
...
def dumpEvent(t,gen,reco):
    ret = '{gqopt:.4E} {gpt:8.2f} | {rqopt:.4E} {rpt:8.2f} | {nsig:4.2f}'
    gqop = t.gen_lep_qOverPt[gen]
    gpt = t.gen_lep_pt[gen]
    rqop = t.lep_qOverPt[reco]
    rpt = t.lep_pt[reco]
    nsig = abs(gpt-rtp) / t.lep_pt_err[reco]
    dumpLog.info(ret.format(**locals()))

def getRefit(t,r,refit):
    if refit=='tuneP':
        return t.lep_tuneP_qOverPt[r]
    elif refit=='global':
        return t.lep_glb_qOverPt[r]
    elif refit=='picky':
        return t.lep_picky_qOverPt[r]
    elif refit=='dyt':
        return t.lep_dyt_qOverPt[r]
    elif refit=='tpfms':
        return t.lep_tpfms_qOverPt[r]
    elif refit=='tracker':
        return t.lep_tk_qOverPt[r]
    else:
        print refit,'not a valid refit option'
        exit()
            ...
            for pt in pts:
                for q in charges:
                    hists[p][x][cat][pt][q] = {
                            ...
                            'p':R.TH1F('p_'+x+'_'+cat+'_'+pt+'_'+q+'_'+p,';'+x+' muon p [GeV];N(#mu)',7,array.array('d',[0,500,1000,1500,2000,2500,3000,4000,6000])),
    ...
    for e in range(t.GetEntries()):
        ...
        if thisEvent==prevEvent or thisEvent==nextEvent:
            ...
            for match in matches:
                gen,reco = match
                if args.log: dumpEvent(t,gen,reco)
                Categories = {
                        'allcat':True,
                    ...
                    for q in charges:
                        for pt in pts:
                            if Categories[cat] and Charge[q] and Pt[pt]:
                                hists['den']['gen'][cat][pt][q]['pt'].Fill(t.gen_lep_pt[gen])
                                hists['den']['gen'][cat][pt][q]['p'].Fill(t.gen_lep_p[gen])
                                hists['den']['gen'][cat][pt][q]['eta'].Fill(t.gen_lep_eta[gen])
                                hists['den']['gen'][cat][pt][q]['phi'].Fill(t.gen_lep_phi[gen])
                                hists['den']['gen'][cat][pt][q]['mass'].Fill(t.gen_dil_mass)
                                hists['den']['gen'][cat][pt][q]['charge'].Fill(sign(t.gen_lep_qOverPt[gen]))
                                #recoMuonQoverPt = getRefit(t,reco,args.refit)
                                if sign(t.gen_lep_qOverPt[gen])!=sign(t.lep_tuneP_qOverPt[reco]):
                                #if sign(t.gen_lep_qOverPt[gen])!=sign(t.lep_dyt_qOverPt[reco]):
                                    hists['num']['gen'][cat][pt][q]['pt'].Fill(t.gen_lep_pt[gen])
                                    hists['num']['gen'][cat][pt][q]['p'].Fill(t.gen_lep_p[gen])
                                    hists['num']['gen'][cat][pt][q]['eta'].Fill(t.gen_lep_eta[gen])
                                    hists['num']['gen'][cat][pt][q]['phi'].Fill(t.gen_lep_phi[gen])
                                    hists['num']['gen'][cat][pt][q]['mass'].Fill(t.gen_dil_mass)
                                    hists['num']['gen'][cat][pt][q]['charge'].Fill(sign(t.gen_lep_qOverPt[gen]))
