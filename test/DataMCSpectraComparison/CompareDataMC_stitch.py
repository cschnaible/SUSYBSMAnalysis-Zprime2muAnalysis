...
parser.add_argument('--do-separate',action='store_true',help='Separate normalizaion region from signal region')
        ...
        2017:{
            ...
            'our':{
                ...
                'FR':{
                    ...
                    #'all':'mc_2017/Data2017-total-jets-BB-BE-EE.root',
                    #'bb':'mc_2017/Data2017-total-jets-BB.root',
                    #'beee':'mc_2017/Data2017-total-jets-BEEE.root',
                    },
                },
            'ourcommonpre':{
        ...
        2018:{
            ...
            'our':{
                ...
                'FR':{
                    ...
                    #'all':'mc/Data2018-total-jets-BB-BE-EE.root',
                    #'bb':'mc/Data2018-total-jets-BB.root',
                    #'beee':'mc/Data2018-total-jets-BE-EE.root',
                    },
                },
            'ourcommonpre':{
...
if args.logx: 
    highMass = np.logspace(np.log10(120),np.log10(args.xmax),45)
    lowMass = np.logspace(np.log10(args.xmin),np.log10(120),9)
    hbins = np.concatenate((lowMass,highMass[1:]))
    xbins = np.concatenate((lowMass,highMass[1:]))
...
hist = R.TH1D('h','',len(hbins)-1,hbins)
hist_low = R.TH1D('h_low','',len(lowMass)-1,lowMass)
hist_high = R.TH1D('h_high','',len(highMass)-1,highMass)
...
totData_low = hist.Clone('totData_low')
totData_high = hist.Clone('totData_high')
wdataHists = {w:{year:{} for year in allyears} for w in ['high','low']}
...
for year in allyears:
    for w in ['high','low']:
        ...
        t = f.Get(info[year][whichMass[w]]['dir']+'/t')
        print info[year][whichMass[w]]['dir']
        dname = 'hdata_'+str(year)+'_'+w
        hdata = hist.Clone(dname)
        print data_sel
        toDraw = dataDraw+'>>'+dname
        print toDraw
        t.Draw(toDraw,data_sel,'pe')
        hdata.SetDirectory(0)
        wdataHists[w][year] = hdata
        wdataHists[w][year].SetDirectory(0)
        if w=='high':
            dataHists[year] = hdata
            totData_high.Add(hdata)
        else:
            dataHists[year].Add(hdata)
            totData_low.Add(hdata)
...
def mc_cut_weight(year,name,w):
    if w=='high':
        mcCutWeight = '('+sel+' && vertex_m>120)'
    elif w=='low':
        mcCutWeight = '('+sel+' && vertex_m<120)'
    if year!=2016 and args.nnpdf30 and name in nnpdf_to_rescale[year][whichMass[w]]:
        if w=='high':
            mcCutWeight += '*('+toNNPDF30[year][whichMass[w]][name][args.category]+'*(gen_res_mass>120))'
        elif w=='low':
            mcCutWeight += '*('+toNNPDF30[year][whichMass[w]][name][args.category]+'*(gen_res_mass<120))'
    if args.order=='nnlo' and args.nnpdf30 and name=='powheg':
        # relies on the nnpdf3.0 reweight for 2017 and 2018 MC
        if w=='high':
            mcCutWeight += '*('+kFunction[whichMass[w]][args.category]+'*(gen_res_mass>120))'
        elif w=='low':
            mcCutWeight += '*(1.)*(gen_res_mass<120)'
...
totMC_low = hist.Clone('totMC_low')
totMC_high = hist.Clone('totMC_high')
#hist_sums = {year:hist.Clone('hmc_'+str(year)) for year in allyears}
hist_sums = {name:hist.Clone(name) for name in mcDrawOrder}
hist_sums_high = {name:hist.Clone(name+'_low') for name in mcDrawOrder}
hist_sums_low = {name:hist.Clone(name+'_high') for name in mcDrawOrder}
whists = {year:{name:{mc:{w:{} for w in ['high','low']} for mc in allMCs[year][name]} for name in mcDrawOrder} for year in allyears}
...
for name in mcDrawOrder:
    ...
    for year in allyears:
        for mc in allMCs[year][name]:
            hists[year][name][mc] = hist.Clone('h'+str(year)+'_'+mc)
            ...
            for w in ['high','low']:
                hmcname = 'h'+str(year)+'_'+mc+'_'+w
                hmc = hist.Clone(hmcname)
                if year==2018:
                    if name in ['amcatnlo','madgraph']:
                        mcTree = mcFile.Get(info[2017][whichMass[w]]['dir']+'/t')
                    else:
                        dirname = 'Our2018MuPrescaledMuonsOppSignNtuple' if w=='low' else 'Our2018MuonsOppSignNtuple'
                        mcTree = mcFile.Get(dirname+'/t')
                else:
                    mcTree = mcFile.Get(info[year][whichMass[w]]['dir']+'/t')
                mcTree.SetAlias('gen_lead_pt','(gen_lep_pt[0]*(gen_lep_pt[0]>gen_lep_pt[1]) + gen_lep_pt[1]*(gen_lep_pt[1]>gen_lep_pt[0]))')
                mcTree.Draw(mc_draw(year)+'>>'+hmcname,mc_cut_weight(year,name,w),'')
                hmc.SetDirectory(0)
                # Scale by (cross section / sum weights) * lumi
                scale_by = info[year]['mc'][mc].cross_section / float(info[year]['mc'][mc].sum_weights)
                scale_by *= info[year]['lumi'] 
                # Scale high mass by Z-peak ratios
                scale_by *= info[year]['ratioZ0'][args.category] if w=='high' else 1.0
                # If desired, scale MC to prescale found in data
                scale_by *= (1./info[year][whichMass[w]]['pre']) if args.prescale_weight=='mc' else 1.0
                # Scale and add
                hmc.Scale(scale_by)

                whists[year][name][mc][w] = hmc
                whists[year][name][mc][w].SetDirectory(0)
                hists[year][name][mc].Add(hmc)
                hists[year][name][mc].SetDirectory(0)
                if w=='low':
                    totMC_low.SetDirectory(0)
                    totMC_low.Add(hmc)
                    hist_sums_low[name].SetDirectory(0)
                    hist_sums_low[name].Add(hmc)
                elif w=='high':
                    totMC_high.SetDirectory(0)
                    totMC_high.Add(hmc)
                    hist_sums_high[name].SetDirectory(0)
                    hist_sums_high[name].Add(hmc)
...
def remove_bins(hist,which='low'):
    if which=='low':
        for b in range(1,hist.GetNbinsX()):
            if hist.GetBinCenter(b) < 120:
                print '*'*15,hist.GetBinCenter(b)
                hist.SetBinContent(i,0)
                hist.SetBinError(i,0)
    elif which=='high':
        for b in range(1,hist.GetNbinsX()+2):
            if hist.GetBinCenter(b) > 120:
                hist.SetBinContent(i,0)
                hist.SetBinError(i,0)
    else:
    ...
    if args.do_paper:
        ...
        for col in paperDrawOrder:
            for name in cols[col]:
                paper_sums[col].Add(hist_sums_high[name])
            ...
            if args.overflow: 
                move_overflow_into_last_bin(paper_sums[col])
            if args.bin_width: 
                divide_bin_width(paper_sums[col])
            elif args.cumulative: 
                paper_sums[col] = cumulative_histogram(paper_sums[col],overflow=False,end=120.)
...
if args.overflow and not args.bin_width and not args.cumulative:
    move_overflow_into_last_bin(totMC)
    move_overflow_into_last_bin(totData)
    move_overflow_into_last_bin(totData_high)
    gdata = poisson_intervalize(totData,bin_width=False,zero_ex=True)
    gdata_low = poisson_intervalize(totData_low,bin_width=False,zero_ex=True)
    gdata_high = poisson_intervalize(totData_high,bin_width=False,zero_ex=True)
if args.bin_width:
    if args.overflow:
        move_overflow_into_last_bin(totMC)
        move_overflow_into_last_bin(totData)
        move_overflow_into_last_bin(totData_high)
    #divide_bin_width(totMC)
    totMC_low_bw = totMC_low.Clone('totMC_low_bw')
    totMC_high_bw = totMC_high.Clone('totMC_high_bw')
    divide_bin_width(totMC_low_bw)
    divide_bin_width(totMC_high_bw)
    gdata = poisson_intervalize(totData,bin_width=args.bin_width,zero_ex=True)
    gdata_low = poisson_intervalize(totData_low,bin_width=args.bin_width,zero_ex=True)
    gdata_high = poisson_intervalize(totData_high,bin_width=args.bin_width,zero_ex=True)
    #divide_bin_width(totData)
    #divide_bin_width(totData_low)
    #divide_bin_width(totData_high)
elif args.cumulative:
    if args.overflow:
        move_overflow_into_last_bin(totMC)
        move_overflow_into_last_bin(totData)
    ...
    if args.do_separate:
        totMC_low = cumulative_histogram(totMC_low,noerr=True)
        totMC_high = cumulative_histogram(totMC_high,noerr=True,end=120.)
    else:
        totMC_low_cum = totMC.Clone('totMC_low_cum')
        remove_bins(totMC_low_cum,which='high')
    totData = cumulative_histogram(totData)
    totData_low = cumulative_histogram(totData_low,begin=120.)
    totData_high = cumulative_histogram(totData_high,end=120.)
    gdata = poisson_intervalize(totData,bin_width=False,zero_ex=True)
    gdata_low = poisson_intervalize(totData_low,bin_width=False,zero_ex=True)
    gdata_high = poisson_intervalize(totData_high,bin_width=False,zero_ex=True)
else:
...
if args.do_paper: 
    ...
    gdata_low.SetMarkerSize(0.9)
    gdata_high.SetMarkerSize(0.9)

# Remove low mass points on high mass graph
def remove_point(graph,which):
    x,y = R.Double(), R.Double()
    for i in range(0,graph.GetN()):
        graph.GetPoint(i, x, y)
        if which=='low':
            if x<120: graph.RemovePoint(i)
        elif which=='high':
            if x>120: graph.RemovePoint(i)

remove_point(gdata_high,'low')
remove_point(gdata_low,'high')

def remove_point_val(graph,val=0):
    x,y = R.Double(), R.Double()
    for i in range(0,graph.GetN()+1):
        graph.GetPoint(i, x, y)
        if y==val: graph.RemovePoint(i)
...
canvas.addMainPlot(pmc_low,addToPlotList=False)
pmc_low.SetLineWidth(1)
pmc_low.SetMarkerStyle(0)
pmc_low.SetFillColor(R.kGray)
pmc_low.SetLineColor(R.kBlack)

if args.do_separate:
    canvas.addMainPlot(pdata_low,addToPlotList=False)
    canvas.addMainPlot(pdata_high)
else:
...
if args.do_stack:
    if args.do_paper:
        ...
        canvas.legend.addLegendEntry(pmc_low)
    else:
...
canvas.mainPad.cd()
normLineMain = R.TLine(120,hmin,120,hmax)
normLineMain.Draw()
...
canvas.ratPad.cd()
normLineRat = R.TLine(120,-1,120,1)
normLineRat.Draw()
canvas.mainPad.cd()
if args.logx: 
