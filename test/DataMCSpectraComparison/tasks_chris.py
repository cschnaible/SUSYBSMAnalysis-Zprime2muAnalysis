...
if cmd == 'setdirs':
    ...
    from SUSYBSMAnalysis.Zprime2muAnalysis.MCSamples import samples17
...
#        do('crab getoutput -d crab/crab_ana_datamc_%(name)s --checksum=no ' % sample)
    ...
    dirs = glob.glob('crab/crab_ana_datamc_Run2017MuonsOnly_SingleMuonRun2017*%(ex)s'%locals())
    for d in dirs:
        print d
    ...
    print cmd
    extra = extra[0] if extra else ''
    dirs = glob.glob('crab/crab_ana_datamc_Run2017MuonsOnly_SingleMuonRun2017*_%s'%extra)
    for d in dirs:
        print d
