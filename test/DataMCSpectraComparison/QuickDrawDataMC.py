...
parser.add_argument('-f','--file',dest='datafile',default='data/ana_datamc_Run2017BCDEF_20190416.root')
parser.add_argument('-d','--dir',default='Our2017MuonsOppSignNtuple')
parser.add_argument('-w','--where',default='test',help='Where to save plots')
...
if args.do_stack:
    ...
    ptW    = Plotter.Plot(hists['tW'],legName=mc_stuff('tW')[0],legType='F')
    pdib   = Plotter.Plot(hists['WW_v2'],legName=mc_stuff('WW_v2')[0],legType='F')
    pwjets = Plotter.Plot(hists['Wjets'],legName=mc_stuff('Wjets')[0],legType='F')
    ptau   = Plotter.Plot(hists['dyInclusive50_amcatnlo'],legName=mc_stuff('dyInclusive50_amcatnlo')[0],legType='F')
else:
