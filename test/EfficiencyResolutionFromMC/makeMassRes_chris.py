...
mrange = [50, 100, 150, 200, 300, 400, 600, 800, 1000, 1300, 1600, 2000, 2500, 3100, 3800, 4500, 5500, 6500, 10000]
...
#outputfile = ROOT.TFile('output_test.root','recreate')
...
def doFitGeneric(hist,output,rap="BB",fit="cruijff",syst=False):
    ...
    for i,h in enumerate(hist):
        ...
        if "cruijff" in fit: 
            ...
            funct.SetParameters(gaus.GetParameter(0), gaus.GetParameter(1), gaus.GetParameter(2), 0.2, 0.1)
...
#            funct.SetParLimits(1, gaus.GetParameter(1)*0.5, gaus.GetParameter(1)*1.5)
        ...
        if syst: 
            if ("cruijff" in fit): 
                print ">>>>>>>>  Using DOUBLE SIDED CRYSTAL BALL for systematics >>>>>>>>"
        ...
        for par in range(funct.GetNpar()-1):
            print funct.GetParName(par+1), funct.GetParameter(par+1)
        ...
        #print nvar, funct.GetNumberFitPoints(), funct.GetNpar()
        #print funct.GetChisquare(), funct.GetNDF(), funct.GetName()
    ...
    for k,f in enumerate(param): 
        ...
        for i in range(0,len(mass)):
            if k==nvals: 
                ...
                f.SetPoint(i,mass[i],pars[i*nvals+k])
                f.SetPointError(i,merr[i],errs[i*nvals+k])
...
#        if os.path.exists("/afs/cern.ch"): os.system("cp /afs/cern.ch/user/g/gpetrucc/php/index.php "+args.output)
    ...
    print "Saving result in: %s" %(path_base)

    makeMassRes(inputfile,path_base,args.funct)
    print "DONE"
